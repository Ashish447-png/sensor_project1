...
from pathlib import Path # for zip file
from src.constant import * #[* >> everything]
...
class DataIngestion:
    ...
    def export_collection_as_dataframe(self, collection_name,db_name):
        try:
            mongo_client = MongoClient(MONGO_DB_URL) # comming from constant

            collection = mongo_client[db_name][collection_name]
            df = pd.DataFrame(list(collection.find()))

            if "_id" in df.columns.to_list():
                df = df.drop(columns= ['_id'], axis = 1) # if _id columns present the drop it

            df.replace({"na": np.nan}, inplace = True) # handel the null value
    ...
    def export_data_into_features_store_file_path(self)-> pd.DataFrame:
        ...
        try:
            ...
            logging.info(f"saving exported data into feature sotre file path: {raw_file_path}")
